<script lang="ts" generics="TSelectMenuType extends 'User' | 'Channel' | 'Role' | 'string'">
 import { findInParents } from '$lib/scripts/util/utils.js';
 import clsx from 'clsx';
 import Options from './Select/Options.svelte';
 import SelectedOption from './Select/SelectedOption.svelte';
 import SelectedOptions from './Select/SelectedOptions.svelte';
 import type { RChannel, RRole, RUser } from '@ayako/bot/src/Typings/Redis';

 type TOption = TSelectMenuType extends 'User'
  ? RUser
  : TSelectMenuType extends 'Channel'
    ? RChannel
    : TSelectMenuType extends 'Role'
      ? RRole
      : string;

 const {
  type = 'string' as TSelectMenuType,
  class: className,
  options,
  maxOpts = 1,
  minOpts = 1,
  required = false,
  searchable = false,
  label,
  id = Math.random().toString(36).substring(7),
  onupdate,
  ontyping,
  selectedOptions = $bindable([])
 }: {
  type?: TSelectMenuType;
  class?: string;
  options: TOption[];
  maxOpts: number;
  minOpts: number;
  required: boolean;
  searchable?: boolean;
  label: string;
  id?: string;
  onupdate?: (v: TOption[]) => void;
  ontyping?: (v: string) => void;
  selectedOptions?: TOption[];
 } = $props();

 const single = maxOpts === 1;

 let element: HTMLDivElement | null = $state(null);
 let expanded = $state(false);
 let selectSearch = $state('');
 let typingDebounce = $state<ReturnType<typeof setTimeout> | null>(null);

 const update = () => {
  if (expanded) return;
  onupdate?.($state.snapshot(selectedOptions) as TOption[]);
 };

 const isSelected = (opt: string | RUser | RChannel | RRole): boolean => {
  if (typeof opt === 'string') {
   return selectedOptions.includes(opt as TOption);
  }

  const optId = (opt as any).id;
  return selectedOptions.some((s) => typeof s !== 'string' && (s as any).id === optId);
 };

 const optionClick = (opt: string | RUser | RChannel | RRole) => {
  if (opt === 'Clear selection') {
   selectedOptions.splice(0, selectedOptions.length);

   update();
   if (single) expanded = false;

   return;
  }

  if (single) {
   selectedOptions.splice(0, selectedOptions.length, opt as TOption);
  } else {
   if (isSelected(opt)) {
    const index = selectedOptions.findIndex((s) => {
     if (typeof opt === 'string') return s === opt;
     return typeof s !== 'string' && (s as any).id === (opt as any).id;
    });
    if (index !== -1) selectedOptions.splice(index, 1);
   } else {
    selectedOptions.push(opt as TOption);
   }
  }

  update();
  if (single) expanded = false;
 };

 const clickWindow = (
  e: (MouseEvent | KeyboardEvent) & { currentTarget: (EventTarget & Window) | HTMLDivElement }
 ) => {
  if (findInParents(e.target as HTMLElement, id)) return;
  expanded = false;
  update();
 };

 const labelClick = (updateState?: boolean) => {
  expanded = updateState !== undefined ? updateState : !expanded;
  update();
 };
</script>

<svelte:window onclick={clickWindow} onkeydown={clickWindow} />

<div {id} class={clsx('relative w-full', className)}>
 <input
  type="text"
  {required}
  value={selectedOptions.length ? JSON.stringify(selectedOptions) : ''}
  class="w-1px h-1px absolute top-full"
  name={id}
  onfocus={() => element?.focus()}
  tabindex={searchable ? 0 : -1}
 />
 {#if searchable}
  <div
   class="w-full relative bg-main-darker rounded-md flex flex-row items-center"
   class:pl-1={selectedOptions.length}
  >
   {#if selectedOptions.length}
    <div
     class="flex flex-row gap-1 items-center"
     class:top-1={!single}
     class:top-2={single}
     class:left-1={!single}
     class:left-2={single}
    >
     {#each selectedOptions as opt, j (j)}
      <SelectedOption {single} {optionClick} {opt} />
     {/each}
    </div>

    <div
     class="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none flex flex-row-reverse"
    >
     <span
      class="i-tabler-chevron-up transition-all duration-100 ease-in-out select-none"
      class:rotate-180={expanded}
      aria-hidden="true"
     ></span>
    </div>
   {/if}

   <input
    bind:this={element}
    bind:value={selectSearch}
    onclick={() => labelClick(true)}
    onkeydown={(e) => {
     if (e.key === 'Enter') {
      labelClick(true);
      return;
     }

     if (typingDebounce) clearTimeout(typingDebounce);

     typingDebounce = setTimeout(() => {
      ontyping?.(selectSearch);
     }, 500);
    }}
    role="combobox"
    tabindex="0"
    class="cursor-pointer bg-transparent rounded-md w-full px-2 py-2 relative text-left placeholder:color-neutral-500 focus:outline-none focus:ring-2 focus:ring-primary-500/50"
    aria-label={label}
    aria-expanded={expanded}
    aria-haspopup="listbox"
    aria-controls={`listbox-${id}`}
    placeholder={label}
    {id}
   />
  </div>
 {:else}
  <div
   bind:this={element}
   onclick={() => labelClick()}
   onkeydown={(e) => (e.key === 'Enter' || e.key === ' ' ? labelClick() : undefined)}
   role="combobox"
   tabindex="0"
   class="cursor-pointer bg-main-darker w-full rounded-md px-2 relative text-left focus:outline-none focus:ring-2 focus:ring-primary-500/50"
   class:pl-1={selectedOptions.length}
   class:py-1={selectedOptions.length}
   class:py-2={!selectedOptions.length}
   aria-label={label}
   aria-expanded={expanded}
   aria-haspopup="listbox"
   aria-controls={`listbox-${id}`}
  >
   <SelectedOptions {selectedOptions} {single} {label} {optionClick} />

   <div class="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none flex flex-row-reverse">
    <span
     class="i-tabler-chevron-up transition-all duration-100 ease-in-out select-none"
     class:rotate-180={expanded}
     aria-hidden="true"
    ></span>
   </div>
  </div>
 {/if}

 <div class="w-full flex flex-col justify-center flex-wrap z-50 relative">
  <Options
   {expanded}
   {options}
   {single}
   {required}
   {selectedOptions}
   {optionClick}
   {selectSearch}
  />
 </div>

 {#if required && !selectedOptions.length}
  <div role="alert" class="text-danger text-xs mt-1">This must have a value</div>
 {/if}

 {#if selectedOptions.length > maxOpts}
  <div role="alert" class="text-danger text-xs mt-1">
   This cannot have more than {maxOpts} values
  </div>
 {/if}

 {#if required && selectedOptions.length < minOpts}
  <div role="alert" class="text-danger text-xs mt-1">
   This cannot have less than {minOpts} values
  </div>
 {/if}
</div>
