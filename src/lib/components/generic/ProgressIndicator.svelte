...
<div class={clsx('progress-indicator', className)}>
 {#if variant === 'linear'}
  ...
  <div class="w-full">
   ...
   <div
    class={clsx('w-full rounded-full overflow-hidden', 'bg-white/10', sizeClasses[size].height)}
    role="progressbar"
   ...
   >
    {#if indeterminate}
     <div class={clsx('h-full rounded-full', colorClasses[color], 'animate-indeterminate')}></div>
    {:else}
     <div
      class={clsx(
