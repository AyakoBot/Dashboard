...
>
 <!-- Illustration/Icon -->
 <div class="mb-6 relative" transition:scale={{ duration: 500, delay: 100 }}>
  ...
  <div
   class="absolute -bottom-1 -left-1 w-2 h-2 bg-primary-500/30 rounded-full animate-pulse"
   style="animation-delay: 0.5s"
  ></div>
  <div
   class="absolute top-1/2 -right-4 w-2 h-2 bg-primary-300/30 rounded-full animate-pulse"
 ...
 <!-- Text Content -->
 <div class="max-w-md">
  <h3 class={clsx('font-semibold text-white mb-2', variant === 'compact' ? 'text-lg' : 'text-xl')}>
   {title}
  </h3>

  {#if description}
   <p class={clsx('text-white/60', variant === 'compact' ? 'text-sm' : 'text-base')}>
    {description}
   </p>
  {/if}
 </div>
