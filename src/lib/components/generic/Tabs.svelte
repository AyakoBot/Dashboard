<script lang="ts">
 ...
 const getTabClasses = (tabId: string) => {
  ...
  return clsx(
   ...
   variant === 'underline' && [
    'pb-4',
    isActive ? 'text-primary-400' : 'text-muted-text hover:text-main-text'
   ],
 ...
 <div class={tabListClasses} role="tablist">
  {#each tabs as tab}
   ...
   >
    ...
    {#if tab.badge}
     <span
      class={clsx('ml-2 px-1.5 py-0.5 text-xs rounded-full', 'bg-danger text-white font-bold')}
     >
      {tab.badge}
     </span>
    {/if}
 ...
 <div class="animate-fade-in">
  {#each tabs as tab}
   <div
    ...
    class={clsx('transition-all duration-300', activeTab === tab.id ? 'block' : 'hidden')}
   >
    {@render tab.content()}
   </div>
  {/each}
