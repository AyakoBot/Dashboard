<script lang="ts">
	...
	const {
		url,
		selected = false
	}: { url: ShortUrlRecord; selected?: boolean } = $props();
	...
	const truncatedForward = $derived(
		url.forward
			.replace(/^https?:\/\//, '')
			.replace(/^www\./, '')
	);
...
>
	<div class="flex flex-wrap items-start justify-between gap-4">
		<div class="space-y-2">
			...
			>
				...
				<span class={clsx(slugCopied ? 'i-tabler-check text-success' : 'i-tabler-copy text-white/40')} aria-hidden="true"></span>
			</button>
		</div>
	...
	<div class="mt-4 rounded-lg border border-white/5 bg-main-darker/60 p-4">
		...
		<div class="mt-2 flex flex-wrap items-center gap-2">
			...
			<button
				type="button"
				onclick={() => openExternal(url.forward)}
	...
	<div class="mt-4 flex flex-wrap items-center gap-3">
		...
		<button type="button" class="btn-danger opacity-60 cursor-not-allowed" title="URL deletion coming soon" aria-disabled="true">
			Delete
		</button>
	</div>
</div>
