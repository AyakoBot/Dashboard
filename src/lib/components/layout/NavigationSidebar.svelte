<script lang="ts">
 ...
 const handleGuildClick = (guild: GuildInfo) => {
  ...
  const iconUrl = guild.icon?.startsWith('http')
   ? guild.icon
...
>
 {#if mobileMenuOpen}
  <span class="i-tabler-x text-xl" aria-hidden="true"></span>
 {:else}
  <span class="i-tabler-menu-2 text-xl" aria-hidden="true"></span>
 {/if}
</button>
...
>
 <div class="p-4 border-b border-white/5">
   ...
   >
    <div class="relative flex-shrink-0">
     <img
      src="/favicon.png"
      alt="Ayako"
      class="w-10 h-10 rounded-xl transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3"
     />
     <div
      class="absolute inset-0 bg-gradient-to-br from-primary-400/20 to-accent-400/20 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
     ></div>
    </div>
    <div class="flex-1 text-left" transition:fade={{ duration: 200 }}>
     <h1 class="text-lg font-semibold text-white group-hover:text-primary-300 transition-colors">
      Ayako Dashboard
     </h1>
  ...
  {:else}
   ...
   >
    <div class="relative flex-shrink-0">
     <div
      class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500/20 to-accent-500/20 flex items-center justify-center transition-all duration-300 group-hover:scale-110"
     >
      <span
       class="i-tabler-chevron-right text-xl text-white transition-transform duration-300 group-hover:translate-x-0.5"
      ></span>
     </div>
    </div>
 ...
 {#if !collapsed || mobileMenuOpen}
  <div class="p-4" transition:fly={{ y: -10, duration: 200 }}>
   <div class="relative">
    <span class="absolute left-3 top-1/2 -translate-y-1/2 i-tabler-search text-white/40 text-sm"
    ></span>
    <input
     id="nav-search"
     type="text"
  ...
  {#each filteredItems as item, i (item.id)}
   ...
   >
    ...
    <span
     class={clsx(
      item.icon,
      'text-xl flex-shrink-0 block',
      collapsed && !mobileMenuOpen && 'mx-auto'
     )}
     aria-hidden="true"
    ></span>
 ...
 {#if guilds.length > 0}
  <div class="p-4 border-t border-white/5">
   ...
   <div class="space-y-3">
    <div
     class={clsx('flex gap-2', collapsed && !mobileMenuOpen ? 'flex-col' : 'flex-row flex-wrap')}
    >
    ...
    >
     <span
      class="i-tabler-grid-dots text-lg group-hover:rotate-90 transition-transform duration-300"
     ></span>
     {#if !collapsed || mobileMenuOpen}
...
{#if mobileMenuOpen}
 <button
  ...
  onkeydown={(e) =>
   (e.key === 'Escape' || e.key === 'Enter' || e.key === ' ') && (mobileMenuOpen = false)}
