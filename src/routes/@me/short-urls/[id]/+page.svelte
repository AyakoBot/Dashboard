	...
	<Card title="Usage Timeline" description="Latest visits sorted from newest to oldest">
		...
		{:then history}
			...
			<div class="rounded-2xl border border-white/10 shadow-inner space-y-3">
				...
				<div class="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-black/20 px-4 py-3">
					<div class="flex items-center gap-3 text-sm text-white/70">
						<span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-white/10 text-xs text-white/60">
							#{index + 1}
						</span>
			...
			{:else}
			<p class="text-sm text-white/40">No visits recorded yet. Share your link and check back soon.</p>
			{/if}
		{:catch error}
