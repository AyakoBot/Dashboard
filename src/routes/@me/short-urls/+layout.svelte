		...
		<section class="grid gap-6 items-start lg:grid-cols-[1.2fr_0.8fr]">
				<Card title="Your Short Links" description="Copy-ready links with quick access to analytics">
					...
					<div class="space-y-4">
						...
						<ShortUrl url={url} selected={selectedId === url.id} />
						{/each}
					</div>
					{:else}
					<div class="rounded-xl border border-white/10 bg-white/5 p-6 text-center text-white/50">
						<p class="text-sm">No short links yet. Contact the Ayako team to enable URL shortening for your account.</p>
					</div>
					{/if}
					</div>
				</Card>
				...
				<Card
					...
					description={selectedId ? 'Detailed usage timeline for the selected short URL' : 'Choose a short link from the list to inspect its usage history'}
				>
					{#if selectedId}
						{@render children()}
					{:else}
						<div class="space-y-3 text-sm text-white/60">
							...
							<p class="text-white/40">Pro tip: keep these analytics open while sharing your link to spot new hits in real time.</p>
						</div>
					{/if}
				</Card>
		</section>
