<script lang="ts">
	...
	const generateToken = async () => {
		...
		const response = await makeRequest({ method: 'POST', path: '/@me/auth/generate' }, {}, fetch)
			.catch(() => null);
	...
	<div class="max-w-6xl mx-auto space-y-10">
		<header class="space-y-4">
			...
			<p class="text-white/60 max-w-3xl">
				Manage your personal API token, review integration guidelines, and explore developer
				resources. Everything in this space follows the same design system shared across the
				dashboard.
			</p>
		</header>
		...
		<section class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
					...
					<div class="space-y-3">
						...
						>
							<div class="flex items-center justify-between gap-3 text-white">
								...
								<span class={copied ? 'i-tabler-check text-success' : 'i-tabler-copy text-white/40'}></span>
							</div>
						</button>
			...
			<Card title="Integration Checklist" description="Keep your implementation aligned with best practices">
				{#snippet children()}
				<ul class="space-y-3">
		...
		<section class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
			<Card title="Developer Resources" description="Helpful destinations for integration success">
				...
				<ul class="space-y-4">
					...
					<li class="flex items-start gap-3 rounded-xl border border-white/10 bg-main-darker/80 p-4 transition hover:border-primary-400/30 hover:shadow-lg">
