...
<div class="min-h-screen bg-gradient-to-br from-main-darkest via-main-darker to-main-dark">
 <div class="pointer-events-none absolute inset-0 overflow-hidden">
  <div
   class="absolute left-[-10rem] top-32 h-80 w-80 rounded-full bg-primary-500/10 blur-3xl"
  ></div>
  <div
   class="absolute right-[-6rem] bottom-24 h-96 w-96 rounded-full bg-accent-purple/10 blur-3xl"
  ></div>
 </div>
     ...
     <div class="space-y-6">
      ...
      <div class="space-y-3">
       ...
       <p class="text-xs text-white/40">
        Showing {filteredReminders.length} of {totalReminders} reminder{totalReminders === 1
         ? ''
         : 's'}
       </p>
      </div>
   ...
   {:else}
    <section class="space-y-6">
     {#each Object.entries(groupedReminders) as [date, dateReminders], index}
      ...
      >
       <div
        class="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-white/50"
       >
       ...
       <div class="mt-4 space-y-3">
        {#each dateReminders.toSorted((a, b) => a.endTime - b.endTime) as reminder}
         <div
          class="rounded-xl border border-white/10 bg-main-darker/70 p-4 transition hover:border-primary-400/40 hover:bg-primary-500/5"
         >
  ...
  >
   <div class="space-y-4">
    <div>
     ...
     ></textarea>
     <div class="mt-1 text-xs text-white/40 text-right">Max 2000 characters</div>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
     <div>
      <label for="date" class="block text-sm font-medium text-white mb-2"> Reminder Date </label>
      <input
       type="date"
       id="date"
       name="date"
     ...
     <div>
      <label for="time" class="block text-sm font-medium text-white mb-2"> Reminder Time </label>
      <input
       type="time"
       id="time"
       name="time"
